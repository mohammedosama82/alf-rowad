        ...
        if (Session["trainer_file_number"] != null)
        {
            ...
            if(txt_post.Text!=null)
            {
                if (FileUpload.HasFile)
                {
                    ...
                    string sql = "insert into posts (trainer_file_number, post, post_upload_date, post_path,count, post_type ) values (@trainer_file_number, @post, @post_upload_date, @post_path,@count, @post_type)";
                    ...
                    txt_post.Text = "";
                    FileUpload.Attributes.Clear();
                    Response.AddHeader("REFRESH", "3;URL=upload_trainers_posts.aspx");
                }
                else
                {
                    string sql = "insert into posts (trainer_file_number, post, post_upload_date, count, post_type ) values (@trainer_file_number, @post, @post_upload_date, @count, @post_type)";
                    SqlCommand cmd = new SqlCommand(sql, con);
                    cmd.Parameters.AddWithValue("@trainer_file_number", Session["trainer_file_number"].ToString());
                    cmd.Parameters.AddWithValue("@post", HttpUtility.HtmlEncode( txt_post.Text));
                    cmd.Parameters.AddWithValue("@post_upload_date", DateTime.Now.ToString("yyyy-MM-dd"));
